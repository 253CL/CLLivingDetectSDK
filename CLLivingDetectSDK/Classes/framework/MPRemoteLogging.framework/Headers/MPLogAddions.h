...
@interface MPLogAdditions : NSObject

/**
 *  UUID、UTDID、clientID、deviceModel，language，接入方无须重写，由mPaaS自动获取。
 */
@property (nonatomic, strong, readonly) NSString *UUID;     // 默认实现为从MPMobileIdentifier获取，如果没有MPMobileIdentifier，会调用系统方法获取UUID
@property (nonatomic, strong, readonly) NSString *UTDID;    // 默认实现为从MPMobileIdentifier获取，如果没有MPMobileIdentifier，会反射式调用UTDID库的[UTDevice utdid]方法
@property (nonatomic, strong, readonly) NSString *clientID; // 默认实现为从MPMobileIdentifier获取，如果没有MPMobileIdentifier，会返回@""
@property (nonatomic, strong) NSString *deviceModel;        // 默认实现为从MPMobileIdentifier获取，如果没有MPMobileIdentifier，会调用系统方法获取设备型号
